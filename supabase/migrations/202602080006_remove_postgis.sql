-- Remove PostGIS from public schema to eliminate advisor/lint noise.
-- This app does not require PostGIS for the MVP (bucketing uses numeric lat/lng).

-- Drop PostGIS-dependent columns and indexes (if they exist).
drop index if exists public.submissions_start_geog_idx;
drop index if exists public.submissions_end_geog_idx;
drop index if exists public.route_clusters_start_geog_idx;
drop index if exists public.route_clusters_end_geog_idx;

alter table public.submissions
  drop column if exists start_geog,
  drop column if exists end_geog;

alter table public.route_clusters
  drop column if exists centroid_start,
  drop column if exists centroid_end;

-- Replace aggregation function to avoid PostGIS calls.
...
-- Keep aggregation locked to service role.
...
-- Drop PostGIS extensions (if installed). Order matters due to dependencies.
