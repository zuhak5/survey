-- Security Advisor + Performance Advisor fixes.
-- Goal: hide extensions from public API surface and improve query performance on time-based scans.

-- Ensure the dedicated extensions schema exists (Supabase convention).
-- Note: Some extensions (e.g. PostGIS) may not support moving schemas after installation.
create schema if not exists extensions;

-- Performance: support time-based scans (e.g. exports/ops/possible future incremental aggregation).
create index if not exists submissions_created_at_idx
  on public.submissions (created_at desc);

-- Avoid temp tables to keep function analysis and execution simpler.
...
begin
  ...
  upserted as (
    ...
    select
      ...
      a.day_of_week,
      a.vehicle_type,
      st_setsrid(st_makepoint(a.centroid_start_lng, a.centroid_start_lat), 4326)::geography,
      st_setsrid(st_makepoint(a.centroid_end_lng, a.centroid_end_lat), 4326)::geography,
...
-- Security: restrict aggregation RPC execution to service role only.
