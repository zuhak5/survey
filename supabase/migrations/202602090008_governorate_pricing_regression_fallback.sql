-- Improve governorate fare fitting:
-- - Lower default min samples so early datasets yield numbers.
-- - Add fallback regression using distance-only when ETA is missing.

create or replace function public.refresh_governorate_pricing(p_min_samples integer default 5)
returns integer
language sql
security definer
  ...
  with
    ...
    coeff_3_rounded as (
      select
        ...
        'multivar'::text as model_kind
      from coeff_3
      where base_iqd_raw is not null and dist_per_km_raw is not null and time_per_min_raw is not null
    ),

    -- Fallback: distance-only regression (no ETA needed).
    coeff_2 as (
      select
        governorate_code,
        count(*)::integer as fit_sample_count,
        regr_intercept(price_iqd, distance_km) as base_iqd_raw,
        regr_slope(price_iqd, distance_km) as dist_per_km_raw
      from baseline_all
      group by governorate_code
    ),
    ...
    coeff_2_rounded as (
      select
        ...
        greatest(0, round(coalesce(base_iqd_raw, 0) / 500) * 500)::integer as base_fare_iqd,
        greatest(0, round(coalesce(dist_per_km_raw, 0) / 50) * 50)::integer as distance_fare_iqd_per_km,
        0::integer as time_fare_iqd_per_min,
        'distance_only'::text as model_kind
      from coeff_2
      where base_iqd_raw is not null and dist_per_km_raw is not null
    ),

    chosen_coeff as (
      select distinct on (g.governorate_code)
        g.governorate_code,
        c.fit_sample_count,
        c.base_fare_iqd,
        c.time_fare_iqd_per_min,
        c.distance_fare_iqd_per_km,
        c.model_kind
      from (select governorate_code from public.governorate_pricing) g
      left join coeff_3_rounded c3 on c3.governorate_code = g.governorate_code
      left join coeff_2_rounded c2 on c2.governorate_code = g.governorate_code
      left join lateral (
        select *
        from (
          select * from coeff_3_rounded where governorate_code = g.governorate_code
          union all
          select * from coeff_2_rounded where governorate_code = g.governorate_code
        ) x
        order by
          -- Prefer multivariate when it meets threshold.
          case when x.model_kind = 'multivar' and x.fit_sample_count >= p_min_samples then 2
               when x.fit_sample_count >= p_min_samples then 1
               else 0 end desc,
          x.fit_sample_count desc
        limit 1
      ) c on true
    ),
      ...
      select
        ...
        percentile_cont(0.5) within group (
          order by abs(
            b.price_iqd - (
              cc.base_fare_iqd
              + cc.distance_fare_iqd_per_km * b.distance_km
              + cc.time_fare_iqd_per_min * coalesce(b.duration_min, 0)
            )
          )
        ) as fit_mae_iqd,
        sqrt(
        ...
        count(*)::integer as fit_sample_count
      from baseline_all b
      join chosen_coeff cc on cc.governorate_code = b.governorate_code
    ...
    surge_stats as (
      select
        ...
        percentile_cont(0.5) within group (
          order by (
            s.price_iqd / greatest(1000, (cc.base_fare_iqd + cc.distance_fare_iqd_per_km * s.distance_km + cc.time_fare_iqd_per_min * coalesce(s.duration_min, 0)))
          )
        ) as surge_p50,
      ...
      join chosen_coeff cc on cc.governorate_code = s.governorate_code
    ...
    stitched as (
      select
        ...
        cc.fit_sample_count as fit_sample_count,
        cc.base_fare_iqd as base_fare_iqd,
        cc.time_fare_iqd_per_min as time_fare_iqd_per_min,
        cc.distance_fare_iqd_per_km as distance_fare_iqd_per_km,
        cc.model_kind,
      ...
      left join chosen_coeff cc on cc.governorate_code = gp.governorate_code
    ...
    updated as (
      ...
      set
        ...
        minimum_fare_iqd = case when coalesce(s.min_sample_count, 0) >= 3 and s.minimum_fare_iqd is not null then s.minimum_fare_iqd else gp.minimum_fare_iqd end,
